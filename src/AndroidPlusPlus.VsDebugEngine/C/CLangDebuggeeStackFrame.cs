      ...
      try
      {
        ...
        if (!m_queriedRegisters)
        {
          ...
          m_debugger.GdbClient.SendCommand (command, delegate (MiResultRecord resultRecord)
          {
            ...
            var registerIdMapping = m_debugger.GdbClient.GetRegisterIdMapping ();
            ...
            for (int i = 0; i < registerValuesCount; ++i)
            {
              ...
              var property = new CLangDebuggeeProperty (m_debugger, this, registerNamePrettified, registerValue);
