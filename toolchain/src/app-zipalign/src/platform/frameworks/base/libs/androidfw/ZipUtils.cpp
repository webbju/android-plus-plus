...
public:
   ...
   long read(unsigned char** nextBuffer, long readSize) const {
       *nextBuffer = mReadBuf;
       long result;
       do { result = ::read(mFd, mReadBuf, readSize); }
...
/*static*/ bool ZipUtils::examineGzip(FILE* fp, int* pCompressionMethod,
    int64_t* pUncompressedLen, int64_t* pCompressedLen, unsigned long* pCRC32)
{
    return examineGzip(fileno(fp), pCompressionMethod,
      pUncompressedLen, pCompressedLen, pCRC32);
}


/*static*/ bool ZipUtils::examineGzip(int fp, int* pCompressionMethod,
    int64_t* pUncompressedLen, int64_t* pCompressedLen, unsigned long* pCRC32)
{
    enum {  // flags
        FTEXT       = 0x01,
    ...
    off64_t curPosn = ::tell64(fp);
    unsigned char buf[8];
    ::lseek64(fp, -8, SEEK_END);
    *pCompressedLen = ::tell64(fp) - curPosn;

    if (::read(fp, buf, 8) != 8)
        return false;
    /* seek back to start of compressed data */
    ::lseek64(fp, curPosn, SEEK_SET);
